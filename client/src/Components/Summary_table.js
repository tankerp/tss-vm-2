        ...
        <tbody>
          ...
          {sitecomponents &&
            sitecomponents.slice(pagesVisited, pagesVisited + componentsperPage).map(site => {
              return (
                <tr onClick={() => handlecomponentSelect(site.pump_no, site)} key={site.pump_no}>
                  <td>{site.project_no}</td>
                  <td>{site.pump_no}</td>
                  <td>{site.state_name}</td>
                  <td>{site.customer_id}</td>
                  <td>{site.developer_name}</td>
                  <td className={site.pump_status === "OFF" ? "text-danger mr-2" : "text-success mr-2"}>{site.pump_status}</td>
